# rpc_server.py
# 继上节的异步 prefork 服务器，下面的代码增加了服务发现、子进程收割、信号处理功能。
# 异步多进程服务共享同样的监听地址，所以只需要父进程注册服务即可。
...
class RPCServer(asyncore.dispatcher):
    ...
    def register_zk(self):
        self.zk = KazooClient(hosts='123.57.155.136:2181')
        self.zk.start()
        self.zk.ensure_path(self.zk_root)  # 创建根节点
        value = json.dumps({"host": self.host, "port": self.port})
        # 创建服务子节点
        self.zk.create(self.zk_rpc, value.encode(encoding="utf-8"), ephemeral=True, sequence=True)
